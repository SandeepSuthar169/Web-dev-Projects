...
import { OrderItems } from "../models/order.models.js"
...
const createOrde = asyncHandler(async (req, res) => {
    ...
    if(!price ||  !quantity ) throw new ApiError(401, "All order fileds are required")
    ...
    const book = await Books.findById(bookId).populate("price")
    console.log(book);     
    if(!book) throw new ApiError(401, "books is  required!")

    let total = book.price * quantity
    if(!price) throw new ApiError(401, "Total quantity price is required!")
